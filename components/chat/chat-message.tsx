...
import { DebugConsole, type DebugData } from "./debug-console";
 ...
 * 
 * Renderiza markdown em tempo real durante streaming para melhor UX.
 * Usa useMemo para otimizar re-renders e manter performance.
 */
function ChatMessageComponent({ message, isStreaming = false, highlightTerm, highlightIndex = 0, messageIndex, debugData }: ChatMessageProps) {
  ...
  // Memoizar conteúdo markdown para otimizar re-renders durante streaming
  const markdownContent = useMemo(() => processedContent, [processedContent]);
  ...
  return (
    <>
        ...
        <div className="flex-1 overflow-hidden min-w-0 pt-1" data-message-index={messageIndex}>
          {isUser ? (
              ...
              isEmpty && isStreaming ? (
                ...
                // Renderização de markdown em tempo real durante streaming
                <div className={cn(
      ...
      {!isUser && !isStreaming && debugMode && debugData && (
