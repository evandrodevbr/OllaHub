...
export class DeepNLPService {
   ...
   */
  static async searchMCPs(
    params: DeepNLPSearchParams,
    retryCount = 0
  ): Promise<DeepNLPResponse> {
    const maxRetries = 3;
    ...
    try {
      ...
      if (params.page_id !== undefined)
        url.searchParams.set("page_id", params.page_id.toString());
      if (params.count_per_page !== undefined)
        url.searchParams.set(
          "count_per_page",
          params.count_per_page.toString()
        );
      ...
      console.log(
        `DeepNLP API Request (attempt ${retryCount + 1}):`,
        url.toString()
      );
      ...
      if (!response.ok) {
        throw new Error(
          `DeepNLP API error: ${response.status} ${response.statusText}`
        );
      }
      ...
      console.error(
        `Error calling DeepNLP API (attempt ${retryCount + 1}):`,
        error
      );
      ...
      throw new Error(
        `Failed to fetch MCPs after ${maxRetries + 1} attempts: ${
  ...
  static async getServerConfig(
    ...
    try {
      ...
      if (!owner || !repo) {
        throw new Error(
          `Invalid MCP ID format: ${id}. Expected format: owner/repo`
        );
      }

      const url = `${DEEPNLP_API_BASE}/server/${owner}/${repo}`;
      console.log(
        `DeepNLP Server Config Request (attempt ${retryCount + 1}):`,
        url
      );
      ...
      if (!response.ok) {
        throw new Error(
          `DeepNLP Server Config API error: ${response.status} ${response.statusText}`
        );
      }
      ...
      console.error(
        `Error getting server config from DeepNLP (attempt ${retryCount + 1}):`,
        error
      );
      ...
      throw new Error(
        `Failed to get server config after ${maxRetries + 1} attempts: ${
          error instanceof Error ? error.message : "Unknown error"
        }`
      );
    }
  ...
  static async getServerTools(id: string, retryCount = 0): Promise<MCPTool[]> {
    ...
    try {
      ...
      console.log(
        `DeepNLP Server Tools Request (attempt ${retryCount + 1}):`,
        url
      );
      ...
      if (!response.ok) {
        throw new Error(
          `DeepNLP Server Tools API error: ${response.status} ${response.statusText}`
        );
      }
      ...
      console.error(
        `Error getting server tools from DeepNLP (attempt ${retryCount + 1}):`,
        error
      );
      ...
      throw new Error(
        `Failed to get server tools after ${maxRetries + 1} attempts: ${
          error instanceof Error ? error.message : "Unknown error"
        }`
      );
    }
  ...
  static transformToMCPProvider(deepnlpItem: any): MCPProvider {
    ...
    return {
      ...
      category,
      tags: deepnlpItem.content_tag_list
        ? deepnlpItem.content_tag_list
            .split(",")
  ...
  static getCategoryMapping(internalCategory: string): string | undefined {
    const mappings =
      this.categoryMap[internalCategory as keyof typeof this.categoryMap];
